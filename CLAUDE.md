...
- **Upstream**: `rtk-ai/rtk` (upstream remote — **fetch only, push disabled**)
...
- `gh` default repo is `algolia/rtk` — never create PRs against upstream

**Upstream push guard** (already set, but re-run if you re-clone):
```bash
git remote set-url --push upstream no-push-to-upstream
gh repo set-default algolia/rtk
```
